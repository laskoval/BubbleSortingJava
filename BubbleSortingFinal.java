...
public class BubbleSortingFinal {
...
public static void errorFile () throws FileNotFoundException{
    ...
    // Create new file
    File file = new File("There is some error in the file for Bubble Sort ");
    ...
    System.err.print("The numbers that were not valid are: ");
 ...
 public static void closeFile(){
  ...
  public static void bubbleSort(int[] array) { 
   //Declare local variables
        ...
        // For loop until true to go through all of array length
        while (swap == true){

        // Set boolean to false to go through rest of loop
          swap = false;
            // For loop to go through each array element
            for( i=1; i < array.length - 1; i++){
              for( j=0; j < array.length - i -1; j++){ 
            // Search through array two numbers at a time 
                if(array[j] > array[j+1]){  

            //Swap the element if the first one is larger than second one
                    temp = array[j];  
                    array[j] = array[j+1];  
                    array[j+1] = temp;
                }
        ...
        System.out.println("");
        System.out.println("Number of comparisons is " + comparison);
    }
